...
async def get_manager_info_text() -> str:
    ...
    for res_code_suffix, managers in sorted(managers_map.items()):
        ...
        for manager in managers:
            ...
            if manager.get('username'):
                user_mention += f" (@{manager['username']})"
...
async def add_id_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if await database.is_manager_in_restaurant(user_id_to_add, res_code):
        ...
        try:
            ...
            text = f"✅ <b>{html.escape(full_name)}</b> успешно добавлен в менеджеры ресторана «{res_name}»."
    ...
    context.user_data.pop('admin_add_res_code', None)
    context.user_data.pop('admin_add_res_name', None)
...
async def handle_admin_delete_candidate(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await edit_admin_message(query,
                             f"Вы уверены, что хотите удалить анкету кандидата <b>{html.escape(candidate_name)}</b> (ID: {candidate_id})? Это действие нельзя будет отменить.",
                             keyboard)
    return AdminState.AWAIT_CANDIDATE_ACTION
...
async def show_stats(update: Update, context: ContextTypes.DEFAULT_TYPE) -> AdminState:
    ...
    if not stats_by_res:
        await edit_admin_message(query, "Статистика пуста.", InlineKeyboardMarkup(
            [[InlineKeyboardButton("⬅️ Назад", callback_data=settings.CALLBACK_ADMIN_BACK)]]))
        return AdminState.MENU
    ...
    survey_names = {
        ...
        'manager_feedback': 'ОС менеджера',
        'candidate_feedback': 'ОС кандидата',
        'exit': 'Exit-интервью',
        'climate': 'Замер климата'
    }
    ...
    for key, total in sorted(totals.items()):
        report.append(f"  - {survey_names.get(key, key)}: <b>{total}</b>")
...
async def broadcast_climate_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> AdminState:
    ...
    if not active_ids:
        await edit_admin_message(query, "Не найдено активных сотрудников для рассылки.", InlineKeyboardMarkup(
            [[InlineKeyboardButton("⬅️ Назад", callback_data=settings.CALLBACK_ADMIN_BACK)]]))
        return AdminState.MENU
    ...
    await edit_admin_message(query,
                             f"Вы уверены, что хотите запустить опрос «Замер климата» для <b>{len(active_ids)}</b> активных сотрудников?",
                             keyboard)
...
async def handle_broadcast_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> AdminState:
    ...
    if not users:
        await edit_admin_message(query, "Ошибка: список получателей пуст.", InlineKeyboardMarkup(
            [[InlineKeyboardButton("⬅️ Назад", callback_data=settings.CALLBACK_ADMIN_BACK)]]))
        context.bot_data['broadcast_in_progress'] = False
        return AdminState.MENU

    await edit_admin_message(query,
                             f"Начинаю рассылку для {len(users)} сотрудников... Это может занять некоторое время.",
                             None)
