...
async def set_user_commands(user_id: int, bot: Bot):
    ...
    elif await database.is_user_a_director(user_id):
        commands = [BotCommand("start", "👑 Панель Управляющего")]
...
async def send_transient_message(context: ContextTypes.DEFAULT_TYPE, chat_id: int, text: str):
    try:
        ...
        if 'transient_messages' not in context.user_data:
            context.user_data['transient_messages'] = []
        context.user_data['transient_messages'].append(message.message_id)
