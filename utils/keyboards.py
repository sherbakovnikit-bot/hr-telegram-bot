...
def get_admin_menu_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("👑 Управление управляющими", callback_data="admin_manage_directors")],
        ...
        [InlineKeyboardButton("📈 Аналитика и HR-отчеты", callback_data="admin_analytics")],
        [InlineKeyboardButton("📊 Запустить замер климата", callback_data="admin_broadcast_climate_start")],
...
def get_director_menu_keyboard(restaurants: List[Dict[str, str]]) -> InlineKeyboardMarkup:
    buttons = []
    for res in restaurants:
        buttons.append(
            [InlineKeyboardButton(f"📍 {res['name']}", callback_data=f"dir_res_{res['code']}")]
        )
    return InlineKeyboardMarkup(buttons)


def get_director_restaurant_management_keyboard(res_code: str) -> InlineKeyboardMarkup:
    buttons = [
        [InlineKeyboardButton("➕ Добавить менеджера", callback_data=f"dir_add_mgr_start_{res_code}")],
        [InlineKeyboardButton("➖ Удалить менеджера", callback_data=f"dir_rem_mgr_start_{res_code}")],
        [InlineKeyboardButton("⬅️ Назад к выбору ресторана", callback_data="director_panel_start")]
    ]
    return InlineKeyboardMarkup(buttons)

def get_analytics_menu_keyboard() -> InlineKeyboardMarkup:
    buttons = [
        [InlineKeyboardButton("📊 Воронка кандидатов", callback_data="analytics_funnel")],
        [InlineKeyboardButton("💰 Зарплатные ожидания", callback_data="analytics_salary")],
        [InlineKeyboardButton("📈 Эффективность источников", callback_data="analytics_sources")],
        [InlineKeyboardButton("🎓 Опыт кандидатов", callback_data="analytics_experience")],
        [InlineKeyboardButton("🚶 Причины увольнений (Exit)", callback_data="analytics_exit")],
        [InlineKeyboardButton("🌡️ Климат в команде", callback_data="analytics_climate")],
        [InlineKeyboardButton("🧑‍🍳 eNPS по должностям", callback_data="analytics_enps_position")],
        [InlineKeyboardButton("⬅️ Назад в админ-панель", callback_data=settings.CALLBACK_ADMIN_BACK)]
    ]
    return InlineKeyboardMarkup(buttons)